      ...
      content={(
        <VStack fullWidth alignItems="start" gap={12}>
          {daoTypes.map(daoType => (
            <Checkbox label={daoTypeName[daoType]} key={daoType} value={value.includes(daoType)} onChange={shouldInclude => onChange(shouldInclude ? [...value, daoType] : value.filter(v => v !== daoType))} />
          ))}
          {!areAllTypesIncluded && <>
            <Line />
