  ...
  return (
    <OverlayMenu
      ...
      content={(
        <VStack fullWidth alignItems="start" gap={12}>
          {proposalStatuses.map(daoType => (
            <Checkbox label={proposalStatusName[daoType]} key={daoType} value={value.includes(daoType)} onChange={shouldInclude => onChange(shouldInclude ? [...value, daoType] : value.filter(v => v !== daoType))} />
          ))}
          {!areAllIncluded && <>
            <Line />
      ...
      renderOpener={({ onClick, ref }) => (
        <div ref={ref}>
          <PrimaryButton kind="secondary" onClick={onClick}>Filter</PrimaryButton>
        </div>
      )}
    />
  )
