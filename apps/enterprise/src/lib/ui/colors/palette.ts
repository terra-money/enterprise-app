...
const paletteHueRangesInterval = paletteHueCoverage.reduce((acc, coverage, index) => {
...
export const generatePaleteCollorGetter =
  ...
  (index: number): HSLA => {
    ...
    const rangeIndex = paletteHueRangesInterval.findIndex((point) => pointOnInterval < point);
    ...
    const hue = range[0] + rangeCoverage * ((pointOnInterval - intervalStart) / intevalLength);
