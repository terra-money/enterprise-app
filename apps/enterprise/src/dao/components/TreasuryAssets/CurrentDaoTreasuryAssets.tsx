      ...
      header={
        <ContentFrame>
          ...
          <HStack fullWidth alignItems="center" justifyContent="space-between">
            <Text weight="semibold">Treasury Holdings</Text>
            {isLoading ? (
              <Spinner />
            ) : assets ? (
              <Text weight="semibold">${formatAmount(sum(assets?.map((asset) => getAssetBalanceInUsd(asset))))}</Text>
            ) : (
              <div />
            )}
          </HStack>
        ...
        <ContentFrame>
          ...
          <VStack gap={24}>
            ...
            <HStack wrap="wrap" alignItems="center" fullWidth justifyContent="space-between">
              ...
              <Actions>
                {assets ? (
                  <PrimaryButton kind="secondary" onClick={() => setShouldShowAllAssets(!shouldShowAllAssets)}>
                    {assetsToDisplay.length < assets.length ? 'Show more' : 'Show less'}
                  </PrimaryButton>
                ) : (
                  <div />
                )}
